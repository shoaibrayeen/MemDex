...
memdex:
  ...
  category: decisions
  description: The config names the variable that holds a key; a pasted key is rejected at load.
  importance: 0.85
...
A value that looks like a real key pasted into `.memdex/config.yaml` is rejected
at load with a `ConfigError` — that file is deliberately committable, and a
